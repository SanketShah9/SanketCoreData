...
+(NSArray *)fetchManagedObjectsForEntity:(NSString *)strEntity withPredicate:(NSPredicate *)predicate withFetchLimit:(BOOL)fetchLimit{
    NSEntityDescription *entitySites = [NSEntityDescription entityForName:strEntity inManagedObjectContext:APPDELEGATE.managedObjectContext];
    NSFetchRequest *fetch = [[NSFetchRequest alloc] init];
    [fetch setEntity:entitySites];
    if(predicate)[fetch setPredicate:predicate];
    if (fetchLimit) {
        [fetch setFetchLimit:1];
    }
    NSError *fetchError;
    NSArray *result =[APPDELEGATE.managedObjectContext executeFetchRequest:fetch error:&fetchError];
    if (fetchError) {
...
+(BOOL)saveManagedObject {
    ...
    if (!result) {
        NSLog(@"Error in adding a new user %@, %@", error, [error userInfo]);
        abort();
    }
    return  result;
}
